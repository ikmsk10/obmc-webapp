...
class SystemDBusConnect : public DBusConnect
{
  public:
    ...
    {}
    ~SystemDBusConnect() noexcept override = default;
    ...
    void connect() override
    {
        ...
        LOG_DEBUG << "Establish DBus connection on system bus";
        initSdBusConnection();
    }
};
...
class RemoteHostDbusConnect : public DBusConnect
{
  ...
  public:
    ...
    {
        LOG_DEBUG << "DBUS hostname:  " << hostname;
    }
    ~RemoteHostDbusConnect() noexcept override = default;

    void connect() override
    {
        auto status = sd_bus_open_system_remote(&dbusConnect, strdup(hostname.c_str()));
        if (status < 0)
        {
            throw std::runtime_error(
                std::string("Can't establish dbus connection. Reason: ") +
        ...
        LOG_DEBUG << "Establish DBus connection to Host: " << hostname;
        initSdBusConnection();
    }
};
