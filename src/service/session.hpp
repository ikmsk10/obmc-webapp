...
struct UserSession
{
    ...
    static std::shared_ptr<UserSession> fromJson(const nlohmann::json& j)
    {
        ...
        for (const auto& element : j.items())
        {
            ...
            {
                LOG_ERROR << "Error reading persistent store.  Property "
            ...
            {
                LOG_ERROR << "Got unexpected property reading persistent file: "
                          << element.key();
                continue;
            }
        }
        ...
        {
            LOG_DEBUG << "Session missing required security "
    ...
    bool isPermitted() const
    {
        ...
        {
            LOG_ERROR << "Unable to check /run/bmc-booting, " << e.what()
                      << "\n";
        }
