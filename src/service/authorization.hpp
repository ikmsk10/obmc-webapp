...
{
    LOG_DEBUG << "[AuthMiddleware] Token authentication";
...
{
    LOG_DEBUG << "[AuthMiddleware] X-Auth-Token authentication";
    ...
    {
        LOG_ERROR << "The 'X-Auth-Token' header is not present";
    }
    return nullptr;
}
    ...
    performCookieAuth(const app::core::RequestPtr& request)
{
        ...
        Fastcgipp::Http::RequestMethod::GET)
    {
        try
        {
            ...
            {
                LOG_DEBUG << "Other header " << other.first << "="
                          << other.second;
            }
        ...
        {
            LOG_ERROR << "The 'X-XSRF-TOKEN' header is not present";
            return nullptr;
        }
    }
#endif
                         ...
                         app::core::ResponseUni& response)
{
    ...
    {
        LOG_WARNING << "[AuthMiddleware] authorization temporary restricted";
    ...
    if (request->isSessionEmpty())
    {
        LOG_WARNING << "[AuthMiddleware] authorization failed";

        // If it's a browser connecting, don't send the HTTP authenticate
        // header, to avoid possible CSRF attacks with basic auth
        // if (app::helpers::utils::requestPrefersHtml(req))
        // {
        //     response->setStatus(app::http::statuses::Code::TemporaryRedirect);
        //     response->setHeader(app::http::headers::location,
        //     "/#/login?next=") res.addHeader("Location",
        //                   "" +
